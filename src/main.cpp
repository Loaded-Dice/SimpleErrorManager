...
//     Serial.printf("  Error 0: %s\n", ERR.getErrorName(WIFI_CONNECT_FAILED));
//     Serial.printf("  Error 1: %s\n", ERR.getErrorName(SENSOR_TIMEOUT));
//     Serial.printf("  Error 2: %s\n", ERR.getErrorName(NTP_SYNC_FAILED));
//     Serial.println();
...
void setup() {
    ...
    // String names demo (only if INCLUDE_ERROR_NAMES is defined)
...
void randomAction() {
    ...
    // Damped by factor of max errors (fewer active = higher probability)
    
    float dampingFactor = 1.0f - (float(n_active)*1.3 / float(MAX_RANDOM_ERRORS));
    uint16_t baseProbability = min(callCount / 3, 100u);  // increases every 3 calls, max 100
    ...
    // 1/40 chance (+/- random 20) to clear active errors
    uint8_t clearChance = 30 + random(-20, 21);  // 20 to 60
